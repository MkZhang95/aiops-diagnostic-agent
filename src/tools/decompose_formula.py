...
def make_decompose_formula_tool(simulator: DataSimulator):
    """Create a decompose_formula tool bound to one diagnostic data source."""

    @tool("decompose_formula")
    def decompose_formula(metric: str, sub_metrics: str) -> str:
        ...
        missing = []
        for name in names:
            data = simulator.query_metrics(name, 0, 3600)
